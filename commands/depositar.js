...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            await fetchHabiticaUser(user.habiticaUserId, user.habiticaToken, "user", "PUT", { 'stats.gp': userGold - qtd })

            user.bank += qtd
            await user.save()

            await interaction.editReply(`✅ Você depositou ${qtd.toFixed(2)} no banco! \nSeu novo saldo é ${user.bank.toFixed(2)}`)
        } catch (error) {
